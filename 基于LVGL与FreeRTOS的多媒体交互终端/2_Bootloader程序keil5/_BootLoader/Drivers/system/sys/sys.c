...
/**
  * @brief  通用的展开式 GPIO 初始化函数
  * @note   Alternate 参数位于第三位。函数内部自动处理时钟使能逻辑。
  * @param  GPIOx: GPIO 端口指针 (如 GPIOA, GPIOB, GPIOC 等)
  * @param  Pin: 引脚号 (如 GPIO_PIN_0, GPIO_PIN_5, GPIO_PIN_ALL)
  * @param  Alternate: 复用功能选择 (如 GPIO_AF1_TIM1; 若非复用模式可填 0)
  * @param  Mode: 工作模式 (如 GPIO_MODE_OUTPUT_PP, GPIO_MODE_AF_PP, GPIO_MODE_INPUT)
  * @param  Pull: 上下拉配置 (GPIO_NOPULL, GPIO_PULLUP, GPIO_PULLDOWN)
  * @param  Speed: 引脚速度 (GPIO_SPEED_FREQ_LOW, GPIO_SPEED_FREQ_VERY_HIGH)
  * @retval None
  */
void config_gpio_init(GPIO_TypeDef *GPIOx, uint32_t Pin, uint32_t Alternate, 
                      uint32_t Mode, uint32_t Pull, uint32_t Speed)
{
    ...
    GPIO_InitStruct.Alternate = Alternate; // 第三个参数
...
/**
 * @brief       根据 USART 实例开启对应的外设时钟
 * @param       USARTx: UART/USART 实例指针 (如 USART1, USART2 等)
 * @retval      无
 */
