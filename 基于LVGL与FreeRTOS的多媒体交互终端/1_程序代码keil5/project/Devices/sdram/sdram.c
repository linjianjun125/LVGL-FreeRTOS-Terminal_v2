...
void sdram_init(void)
{
    ...
    config_gpio_init(GPIOC, ConfigIO_Pin0 | ConfigIO_Pin2 | ConfigIO_Pin3, 		2, 0, 2, 1); 			/* PC0/2/3 				复用功能输出 */
    config_gpio_init(GPIOD, 3 << 0 | 7 << 8 | 3 << 14, 				2, 0, 2, 1); 						/* PD0/1/8/9/10/14/15 	复用功能输出 */
    config_gpio_init(GPIOE, 3 << 0 | 0X1FF << 7, 					2, 0, 2, 1); 						/* PE0/1/7~15 			复用功能输出 */
    config_gpio_init(GPIOF, 0X3F << 0 | 0X1F << 11,					2, 0, 2, 1);  						/* PF0~5/11~15 			复用功能输出 */
    config_gpio_init(GPIOG, 7 << 0 | 3 << 4 | ConfigIO_Pin8 | ConfigIO_Pin15,	2, 0, 2, 1);  			/* PG0~2/4/5/8/15 		复用功能输出 */
    config_afio_init(GPIOC, ConfigIO_Pin0 | ConfigIO_Pin2 | ConfigIO_Pin3, 		12);      				/* PC0,2,3              AF12 */
    config_afio_init(GPIOD, 3 << 0 | 7 << 8 | 3 << 14, 				12);                    			/* PD0/1/8/9/10/14/15   AF12 */
    config_afio_init(GPIOE, 3 << 0 | 0X1FF << 7, 					12);                    			/* PE0/1/7~15           AF12 */
    config_afio_init(GPIOF, 0X3F << 0 | 0X1F << 11, 				12);                   				/* PF0~5/11~15          AF12 */
    config_afio_init(GPIOG, 7 << 0 | 3 << 4 | ConfigIO_Pin8 | ConfigIO_Pin15, 12);   					/* PG0~2/4/5/8/15       AF12 */

	uint32_t sdctrlreg = 0;
    sdctrlreg |= 1 << 0;                /* 9位列地址 */
