...
uint16_t g_norflash_type = W25Q256;    /* 默认是BY25Q256 */
volatile uint8_t g_norflash_addrw = 2;  /* SPI FLASH地址位宽 2, 表示24bit地址宽度 3, 表示32bit地址宽度*/
...
/**
 * @brief   写SPIFLASH 在指定地址开始写入指定长度的数据
 * @param   pbuf: 数据存储区
 * @param   addr: 开始写入的地址(最大32bit)
 * @param   datalen : 要写入的字节数(最大65535)
 * @retval  无
 */
void norflash_write_data(uint8_t *pbuf, uint32_t addr, uint16_t datalen)
{
    uint32_t secpos;
    uint16_t secoff;
    uint16_t secremain;
    ...
    while (1)
    {
        norflash_read_data(norflash_buf, secpos * 4096, 4096);   /* 读出整个扇区的内容 */
...
/**
 * @brief   数据擦除
 * @param   addr: 开始擦除的地址(最大32bit)
 * @param   datalen: 要擦除的字节数(最大65535)
 * @retval  无
 */
void norflash_erase_data(uint32_t addr, uint32_t datalen)
{
    uint32_t sector_addr = addr / 4096;      								// 扇区地址
    uint32_t sector_start_residue = 4096 - addr % 4096;   					// 开始扇区剩余大小
	uint32_t sector_num = (datalen - sector_start_residue) / 4096;			// 完整扇区的数量
	uint32_t sector_end_residue = (datalen - sector_start_residue) % 4096;	// 最后扇区剩余大小
	
	printf("[info] norflash_erase_data 正在擦除数据,约%d个扇区\n", datalen/4096);
	if(sector_start_residue > 0)
	{
		norflash_read_data(g_norflash_buf, sector_addr * 4096, 4096);
		norflash_erase_sector(sector_addr);
		norflash_write_nocheck(g_norflash_buf + 4096 - sector_start_residue, sector_addr * 4096, sector_start_residue);  
		sector_addr++;
	}
	...
	if(sector_end_residue > 0)
	{
		norflash_read_data(g_norflash_buf, sector_addr * 4096, 4096);
		norflash_erase_sector(sector_addr);
		norflash_write_nocheck(g_norflash_buf, sector_addr * 4096, sector_end_residue);  
	}
 ...
 * @retval      无
 */
static void norflash_write_sr(uint8_t regno, uint8_t sr)
{
    uint8_t command = 0;
