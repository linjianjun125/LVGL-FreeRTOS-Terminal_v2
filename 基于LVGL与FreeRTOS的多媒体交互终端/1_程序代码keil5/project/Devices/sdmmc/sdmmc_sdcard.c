...
SD_Error sd_init(void)
{
	...
	/*!< GPIO 配置 >!*/
    config_gpio_init(SDMMC1_D0_GPIO, SDMMC1_D0_PIN,2, 0, 1, 1);   
	config_gpio_init(SDMMC1_D1_GPIO, SDMMC1_D1_PIN,2, 0, 1, 1);
	config_gpio_init(SDMMC1_D2_GPIO, SDMMC1_D2_PIN,2, 0, 1, 1);
	config_gpio_init(SDMMC1_D3_GPIO, SDMMC1_D3_PIN,2, 0, 1, 1);
	config_gpio_init(SDMMC1_CLK_GPIO, SDMMC1_CLK_PIN,2, 0, 1, 1);
	config_gpio_init(SDMMC1_CMD_GPIO, SDMMC1_CMD_PIN,2, 0, 1, 1);
	
	/*!< AFIO 配置 >!*/
    config_afio_init(SDMMC1_D0_GPIO, SDMMC1_D0_PIN, 12);
	config_afio_init(SDMMC1_D1_GPIO, SDMMC1_D1_PIN, 12);	
	config_afio_init(SDMMC1_D2_GPIO, SDMMC1_D2_PIN, 12);
	config_afio_init(SDMMC1_D3_GPIO, SDMMC1_D3_PIN, 12);
	config_afio_init(SDMMC1_CLK_GPIO, SDMMC1_CLK_PIN, 12);
	config_afio_init(SDMMC1_CMD_GPIO, SDMMC1_CMD_PIN,12);  
    ...
    if (errorstatus == SD_OK)	errorstatus = sdmmc_initialize_cards(); 									/* 初始化SD卡 */
    if (errorstatus == SD_OK)	errorstatus = sdmmc_get_card_info(&g_sd_card_info);  						/* 获取卡信息 */
    if (errorstatus == SD_OK)	errorstatus = sdmmc_select_deselect((uint32_t)(g_sd_card_info.RCA << 16));  /* 选中SD卡 */
    if (errorstatus == SD_OK)	errorstatus = sdmmc_wide_bus_operation(1);  								/* 4位宽度,如果是MMC卡,则不能用4位模式 */
