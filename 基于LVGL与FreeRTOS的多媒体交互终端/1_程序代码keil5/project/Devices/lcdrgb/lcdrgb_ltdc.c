...
#include "../../system/debug/debug.h"
	...
	extern lv_color_t lvgl_buf_3_1[];

void lcdrgb_ltdc_init(void)
{
    LCDRGB_LTDC_ENABLE();

    config_gpio_init(LCDRGB_LTDC_HSYNC_GPIO, LCDRGB_LTDC_HSYNC_PIN, 2, 0, 2, 1);
    config_gpio_init(LCDRGB_LTDC_VSYNC_GPIO, LCDRGB_LTDC_VSYNC_PIN, 2, 0, 2, 1);
    config_gpio_init(LCDRGB_LTDC_DE_GPIO, LCDRGB_LTDC_DE_PIN, 2, 0, 2, 1);
    config_gpio_init(LCDRGB_LTDC_CLK_GPIO, LCDRGB_LTDC_CLK_PIN, 2, 0, 2, 1);
    config_gpio_init(GPIOG, ConfigIO_Pin6 | ConfigIO_Pin11, 2, 0, 2, 1);   
    config_gpio_init(GPIOH, 0X7F << 9,2, 0, 2, 1);   						
    config_gpio_init(GPIOI, 7 << 0 | 0XF << 4,2, 0, 2, 1);   				
	...
	//ltdc_layer_parameter_config(LTDC_USE_LAYER, (uint32_t)lvgl_buf_3_1, LTDC_PIXFORMAT_RGB565, 255, 0, 4, 5, 0X000000);
    ltdc_layer_window_config(LTDC_USE_LAYER, 0, 0, LTDC_WIDTH, LTDC_HEIGHT);   

//    /* 2. 在 NVIC 中使能 LTDC 中断，设置优先级 */
//	LTDC->IER |= 1 << 3; 
//	config_nvic_init(5, 0, LTDC_IRQn);  
}
